...
def main():
    ...
    if args.model in ['alphabet', 'both'] and dataset_type == 'alphabet':
        if os.path.exists(model_alpha_path):
            print('\n' + '=' * 80)
            print('Evaluating ALPHABET model:', model_alpha_path)
            print('=' * 80)
            evaluate_model(model_alpha_path, X_test, y_test, SIGN_LABELS_ALPHA, label_order=label_order)
        else:
            print('Alphabet model not found at', model_alpha_path)
